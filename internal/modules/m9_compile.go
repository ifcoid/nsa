...
// AI disclosure yang AKURAT (COPE/Elsevier): sistem ini memakai LLM sebagai alat bantu
// keputusan (decision-support) untuk skrining, ekstraksi, penilaian risiko bias/QA, dan
// sintesis — SELALU dengan verifikasi manusia (HITL) di setiap gerbang, dan reliabilitas
// antar-penilai diukur (Cohen's κ). Menyatakan "AI tidak dipakai untuk analisis" akan
// KELIRU dan berisiko integritas; disclosure harus jujur tentang peran AI + kendali manusia.
const aiDeclaration = `Large language models (LLMs) were used as decision-support tools during title/abstract screening, full-text screening, data extraction, quality/risk-of-bias appraisal, and evidence synthesis. In every case the AI produced proposals that were reviewed, verified, and finalized by the human reviewer(s) through explicit human-in-the-loop gates; dual independent AI raters were used with inter-rater agreement quantified (Cohen's kappa), and the AI was never the sole judge. Symbolic, criteria-derived rules were combined with the AI's neural judgement (neuro-symbolic). A complete, auditable provenance trail of every AI call (model, prompt, output) is retained and available as supplementary material. All final methodological decisions, interpretations, and wording are the responsibility of the author(s).`

const aiDeclarationID = `Model bahasa besar (LLM) digunakan sebagai alat bantu keputusan (decision-support) pada penyaringan judul/abstrak, penyaringan teks lengkap, ekstraksi data, penilaian kualitas/risiko bias, dan sintesis bukti. Pada setiap tahap, AI menghasilkan usulan yang ditinjau, diverifikasi, dan difinalkan oleh reviewer manusia melalui gerbang human-in-the-loop yang eksplisit; dua penilai AI independen dipakai dengan kesepakatan antar-penilai diukur (Cohen's kappa), dan AI bukan hakim tunggal. Aturan simbolik berbasis-kriteria dipadukan dengan penilaian neural AI (neuro-symbolic). Jejak provenance lengkap dan dapat diaudit untuk setiap panggilan AI (model, prompt, keluaran) tersimpan dan tersedia sebagai materi suplementer. Seluruh keputusan metodologis akhir, interpretasi, dan kata akhir adalah tanggung jawab penulis.`
